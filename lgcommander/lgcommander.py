...
def getsessionid(lgtv):
    conn = http.client.HTTPConnection( lgtv["ipaddress"], port=8080, timeout=4)
...
def sendcommand(lgtv, cmdcode):
    conn = http.client.HTTPConnection( lgtv["ipaddress"], port=8080, timeout=8)
...
def querycommand(lgtv, querycode):
    conn = http.client.HTTPConnection( lgtv["ipaddress"], port=8080, timeout=2)
    queryText = "<?xml version=\"1.0\" encoding=\"utf-8\"?><command><session>" \
                + lgtv["session"]  \
                + "</session><type>HandleKeyInput</type><value>" \
                + querycode \
                + "</value></command>"
#    conn = requests.get(lgtv["ipaddress"] + ":8080/hdcp/api/dtv_wifirc", data=queryText, headers=headers, timeout=5)
    conn.request("GET", "/hdcp/api/dtv_wifirc", queryText, headers=headers)
    return conn.getresponse()
